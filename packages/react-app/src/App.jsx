...
function App(props) {
  ...
  if (targetNetwork && targetNetwork.name && targetNetwork.name != "localhost") {
    BACKEND_URL = "https://multisig-wallet-sanjaydefi.herokuapp.com/";
  }
  ...
  // MultiSigBuilder Events
  const ownersMultiSigEvents = useEventListener(readContracts, "MultiSigBuilder", "WalletOwners", localProvider, 1);
  console.log("ownersMultiSigEvents: ", ownersMultiSigEvents);
  ...
  useEffect(() => {
    if (address) {
      let multiSigsForUser = ownersMultiSigEvents.reduce((filtered, createEvent) => {
        if (createEvent.args.owners.includes(address) && !filtered.includes(createEvent.args.walletAddress)) {
          filtered.push(createEvent.args.walletAddress);
        }
        return filtered;
      }, []);
  ...
  const signaturesRequiredContract = useContractReader(readContracts, contractName, "numSignaturesRequired");
  ...
  useEffect(() => {
    async function getContractValues() {
      const latestSignaturesRequired = await readContracts.MultiSigWallet.numSignaturesRequired();
  ...
  console.log("📟 executeTransactionEvents:", allExecuteTrandsactionEvents);
  ...
  console.log("📟 ownerEvents:", allOwnerEvents);
  ...
  /* -----------------------End of MultiSigWallet Contract--------------------------- */
  //
